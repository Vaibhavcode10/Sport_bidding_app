...
export const LiveAuctionViewer: React.FC = () => {
  ...
  if (!hasActiveAuction) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-indigo-900 to-purple-900 flex items-center justify-center">
        <div className="bg-white/10 backdrop-blur-md rounded-xl p-12 text-center max-w-md">
          <div className="text-6xl mb-4">📺</div>
          <h2 className="text-2xl font-bold text-white mb-4">No Live Auction</h2>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-indigo-900 to-purple-900 p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Auctioneer: {session?.auctioneerName}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            {ledger && currentPlayer ? (
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 text-center">
                <div className="text-sm text-gray-400 mb-2">CURRENT PLAYER</div>
                ...
                <h2 className="text-2xl font-bold text-white mb-1">{ledger.playerName}</h2>
                <p className="text-purple-300 mb-4">{currentPlayer.role}</p>

                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="bg-white/5 rounded-lg p-3">
                    <div className="text-gray-400">Base Price</div>
                    <div className="text-green-400 font-bold">{formatCurrency(ledger.basePrice)}</div>
                  </div>
                  <div className="bg-white/5 rounded-lg p-3">
                    <div className="text-gray-400">Status</div>
                    <div className={`font-bold ${
                      ledger.state === 'LIVE' ? 'text-green-400' :
                      ledger.state === 'PAUSED' ? 'text-yellow-400' :
                      'text-gray-400'
                    }`}>
                      {ledger.state}
                    </div>
              ...
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 text-center">
                <div className="text-6xl mb-4">⏳</div>
                <h2 className="text-xl font-bold text-white mb-2">Waiting for Next Player</h2>
                <p className="text-gray-400">The auctioneer is selecting the next player</p>
              </div>
            )}

            {/* Teams Overview */}
            <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 mt-4">
              <h3 className="text-sm font-semibold text-gray-400 mb-3">PARTICIPATING TEAMS</h3>
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 rounded-lg ${
                        isLeading ? 'bg-yellow-500/20 border border-yellow-500/50' : 'bg-white/5'
                      }`}
                    >
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-gray-400 text-sm">{formatCurrency(team.purseRemaining)}</span>
                      </div>
                    </div>
                  );
                })}
              ...
              {ledger && (
                <div className="text-center">
                  ...
                  {ledger.highestBidder ? (
                    <div className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-yellow-500/30 to-orange-500/30 border border-yellow-500/50 rounded-full">
          ...
          <div className="lg:col-span-1">
                  ...
                  {[...ledger.bidHistory].reverse().map((bid, index) => (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <div className="text-white font-medium">{bid.teamName}</div>
                          <div className="text-xs text-gray-400">
                            {new Date(bid.timestamp).toLocaleTimeString()}
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-green-400 font-bold">{formatCurrency(bid.bidAmount)}</div>
                          {bid.isJumpBid && (
                            <span className="text-xs text-purple-300">Jump</span>
                          )}
                        </div>
                      </div>
                    </div>
            ...
            {/* Auction Progress */}
            <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 mt-4">
              <h3 className="text-sm font-semibold text-gray-400 mb-3">AUCTION PROGRESS</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/5 rounded-lg p-3 text-center">
                  <div className="text-2xl font-bold text-white">{session?.completedPlayerIds.length || 0}</div>
                  <div className="text-xs text-gray-400">Completed</div>
                </div>
                <div className="bg-white/5 rounded-lg p-3 text-center">
                  <div className="text-2xl font-bold text-white">{session?.playerPool.length || 0}</div>
        ...
        {/* User Info Banner */}
        <div className="mt-6 bg-white/5 rounded-lg p-3 text-center text-gray-400 text-sm">
          {user ? (
            <>Viewing as {user.role}: {user.name || user.username}</>
          ) : (
            <>Viewing as guest</>
          )}
          <span className="ml-2 px-2 py-1 bg-gray-700 rounded text-xs">Read Only</span>
        </div>
      </div>
    </div>
  );
