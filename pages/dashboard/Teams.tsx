  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-4xl font-black bg-gradient-to-r from-primary-500 via-purple-500 to-pink-500 bg-clip-text text-transparent mb-2">
            {sport.charAt(0).toUpperCase() + sport.slice(1)} Teams
          </h2>
          <p className="text-gray-600 dark:text-slate-400 text-sm">Create and manage teams bidding for players</p>
        </div>
      </div>
      ...
      {(user?.role === 'admin' || user?.role === 'auctioneer') && (
        <div className="flex justify-between items-center">
          <p className="text-gray-600 dark:text-slate-400 font-semibold">{teams.length} Teams Registered</p>
          <button
      ...
      {(user?.role === 'admin' || user?.role === 'auctioneer') && showForm && (
        <div className="bg-white dark:bg-gradient-to-br dark:from-slate-800/50 dark:to-slate-900/50 border border-gray-200 dark:border-slate-700/50 backdrop-blur-xl rounded-2xl p-8 animate-in fade-in slide-in-from-top shadow-lg">
          <h3 className="text-2xl font-bold mb-6 bg-gradient-to-r from-amber-500 to-orange-500 bg-clip-text text-transparent">
      ...
      {teams.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {teams.map((team, idx) => {
            ...
            return (
              <div
                key={team.id}
                className="group bg-white dark:bg-gradient-to-br dark:from-slate-800/60 dark:to-slate-900/60 hover:bg-gray-50 dark:hover:from-slate-800 dark:hover:to-slate-800 border border-gray-200 dark:border-slate-700 hover:border-amber-500/50 rounded-2xl p-6 transition-all transform hover:scale-105 hover:shadow-2xl hover:shadow-amber-500/20 backdrop-blur-xl animate-in fade-in slide-in-from-bottom shadow-md"
                style={{ animationDelay: `${idx * 50}ms` }}
              >
                {/* Team Header */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <h3 className="text-2xl font-black text-gray-900 dark:text-white group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-amber-500 group-hover:to-orange-500 group-hover:bg-clip-text transition-all mb-2">
                      {team.name}
                    </h3>
                    <div className="space-y-1">
                      <p className="text-sm text-gray-600 dark:text-slate-400">
                        <span className="text-amber-600 dark:text-amber-400 font-semibold">Owner:</span> {team.owner}
                      </p>
                      {user?.role === 'admin' && team.city && (
                        <p className="text-xs text-gray-500 dark:text-slate-500">
                          <span className="text-purple-600 dark:text-purple-400 font-semibold">City:</span> {team.city}
                        </p>
                      )}
                      {user?.role === 'admin' && team.auctioneerName && (
                        <p className="text-xs text-gray-500 dark:text-slate-500">
                          <span className="text-blue-600 dark:text-blue-400 font-semibold">Auctioneer:</span> {team.auctioneerName}
                        </p>
                      )}
                      {user?.role === 'admin' && team.founded && (
                        <p className="text-xs text-gray-500 dark:text-slate-500">
                          <span className="text-green-600 dark:text-green-400 font-semibold">Founded:</span> {team.founded}
                        </p>
                      )}
                    </div>
                  </div>
                ...
                {user?.role === 'admin' && (team.stadium || team.capacity || team.description) && (
                  <div className="space-y-2 mb-4 pb-4 border-b border-gray-200 dark:border-slate-700">
                    <h4 className="text-sm font-bold text-cyan-600 dark:text-cyan-400 mb-2">🏟️ Facility Details</h4>
                    {team.stadium && (
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600 dark:text-slate-400 text-xs">Stadium:</span>
                        <span className="text-gray-900 dark:text-white text-xs font-semibold">{team.stadium}</span>
                      </div>
                    )}
                    {team.capacity && (
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600 dark:text-slate-400 text-xs">Capacity:</span>
                        <span className="text-gray-900 dark:text-white text-xs font-semibold">{team.capacity.toLocaleString()}</span>
                      </div>
                    )}
                    {team.description && (
                      <div className="mt-2">
                        <p className="text-xs text-gray-600 dark:text-slate-400 leading-relaxed">{team.description}</p>
                      </div>
                    )}
                    {team.createdAt && (
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600 dark:text-slate-400 text-xs">Established:</span>
                        <span className="text-gray-900 dark:text-white text-xs font-semibold">{team.createdAt}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="space-y-3 mb-4 pb-4 border-b border-gray-200 dark:border-slate-700">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xl font-black text-emerald-600 dark:text-emerald-400">${(team.totalPurse / 1000000).toFixed(1)}M</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 dark:text-slate-400 text-sm">Spent</span>
                    <span className="text-lg font-bold text-red-600 dark:text-red-400">${(spentBudget / 1000000).toFixed(1)}M</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 dark:text-slate-400 text-sm">Remaining</span>
                    <span className="text-lg font-bold text-amber-600 dark:text-amber-400">${(team.purseRemaining / 1000000).toFixed(1)}M</span>
                  </div>

                  {/* Budget Progress Bar */}
                  <div className="w-full bg-gray-200 dark:bg-slate-700/50 rounded-full h-3 overflow-hidden border border-gray-300 dark:border-slate-600">
                    <div
                ...
                <div className="grid grid-cols-2 gap-4 mb-4 pb-4 border-b border-gray-200 dark:border-slate-700">
                  <div className="bg-blue-100 dark:bg-blue-600/20 rounded-lg p-3 text-center border border-blue-300 dark:border-blue-500/30">
                    <p className="text-2xl font-black text-blue-600 dark:text-blue-400">{team.playerCount}</p>
                    <p className="text-xs text-gray-600 dark:text-slate-400 mt-1">Squad Size</p>
                  </div>
                  <div className="bg-purple-100 dark:bg-purple-600/20 rounded-lg p-3 text-center border border-purple-300 dark:border-purple-500/30">
                    <p className="text-2xl font-black text-purple-600 dark:text-purple-400">{team.wins}W</p>
                    <p className="text-xs text-gray-600 dark:text-slate-400 mt-1">Record</p>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  {(user?.role !== 'admin' && user?.role !== 'auctioneer') && (
                    <button
                      className="flex-1 px-4 py-2 bg-gradient-to-r from-gray-200 to-gray-300 dark:from-slate-600 dark:to-slate-500 text-gray-500 dark:text-slate-300 rounded-lg text-xs font-bold cursor-not-allowed"
                      disabled
                    >
                      👁️ View Only
                    </button>
        ...
        <div className="text-center py-16 bg-white dark:bg-gradient-to-br dark:from-slate-800/30 dark:to-slate-900/30 border border-gray-200 dark:border-slate-700 rounded-2xl shadow-sm">
          ...
          <p className="text-gray-600 dark:text-slate-400 text-lg font-semibold">No teams found</p>
          <p className="text-gray-500 dark:text-slate-500 text-sm mt-2">Create teams to start the auction for {sport}</p>
        </div>
      )}
